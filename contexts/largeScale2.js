...
export const analyseLargeScaleDomain = async (url, browser) => {
    ...
    for(let i = 0; i < filtredLinks.length && successfullLinksCounter < requiredNumberOfLinks && retryCounter < retryAmount; i+=windowSize) {
        ...
        let links = [];
        let startIndex = i;
        for(let j = startIndex; j < startIndex + windowSize && j < filtredLinks.length; j++) {
            links.push(filtredLinks[j]);
        }
        ...
        for(let link of links) {
            ...
            if(fs.existsSync(filename)) {
                ...
                linkData.push({fixedLink, filename});
            }
        }

        let canCrawlTasks = [];
        for(let link of linkData) {
            canCrawlTasks.push(robots.canCrawl(link.fixedLink));
        }
        let canCrawlResults = await Promise.allSettled(canCrawlTasks);
