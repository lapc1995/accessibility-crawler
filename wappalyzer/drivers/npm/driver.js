...
class Site {
  ...
  addAnalyzedUrl(url, status) { 
    this.analyzedUrls[url] = {
      status,
    }
  }

  getAnalyzedUrls() {
    return this.analyzedUrls;
  }

  addToXHRDebounce(hostname) {
      ...
      if (request.resourceType() === 'xhr') {
        ...
        if (!xhrDebounce.includes(hostname)) {
          ...
          setTimeout(async () => {
            ...
            this.analyzedXhr[this.url.hostname] =
              this.analyzedXhr[this.url.hostname] || []

            if (!this.analyzedXhr[this.url.hostname].includes(hostname)) {
              this.analyzedXhr[this.url.hostname].push(hostname)

              await this.onDetect(this.url, analyze({ xhr: hostname }))
            }
          }, 1000)
        }
      }
  ...
  async OnResponse(response, page) {
    ...
    try {
      if (
        response.status() < 300 &&
        response.frame().url() === this.url.href &&
    ...
    try {
      if (response.url() === this.url.href) {
        this.analyzedUrls[this.url.href] = {
        ...
        if (response.status() >= 300 && response.status() < 400) {
          if (headers.location) {
            const _url = new URL(headers.location.slice(-1), this.url)
        ...
        await this.onDetect(this.url, analyze({ headers, certIssuer }))
  ...
  async goto(page = null, url) {
    ...
    //Removed this because event handleing is being registered before this is called
    /*
...
*/
  ...
  async analyze(page, url = this.originalUrl, index = 1, depth = 1) {
